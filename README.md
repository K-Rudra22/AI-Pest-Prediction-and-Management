...
India loses **15–25% of agricultural produce annually** due to unpredictable pest infestations. Pest management decisions today are largely reactive and experience-based, meaning farmers realize pest attacks only after visible damage, by which time losses are unavoidable.
...
> **Note:** Delete the `__pycache__` folder before committing — add it to `.gitignore` to keep the repo clean.
